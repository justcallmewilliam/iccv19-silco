...
class Evaluator:
    def GetPascalVOCMetrics(self,
                            boundingboxes,
                            IOUThreshold=0.5,
                            method=MethodAveragePrecision.EveryPointInterpolation, area_thres = [0,1],):
        ...
        for c in classes:
            ...
            #dongzhuoyao
            gts = [g for g in gts if
                   (g[-1][2]-g[-1][0])*(g[-1][3]-g[-1][1])/90000 >area_thres[0]
                   and
                   (g[-1][2] - g[-1][0]) * (g[-1][3] - g[-1][1]) / 90000 < area_thres[1]
                   ]
    ...
    def PlotPrecisionRecallCurve(self,
                                 ...
                                 savePath=None,
                                 showGraphic=True, area_thres=[0,1]):
        ...
        Returns:
            ...
            dict['total FP']: total number of False Negative detections;
        """
        results = self.GetPascalVOCMetrics(boundingBoxes, IOUThreshold, method,area_thres=area_thres)
        result = None
